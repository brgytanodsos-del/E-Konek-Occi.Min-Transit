...
export const Panel3 = ({ isSuperAdmin }: Panel3Props) => {
  ...
  const handleFerryBookingSubmit = (e: React.FormEvent) => {
    ...
    if (isOnline) {
      ...
      // Update seats remaining (optimistic + Firestore)
      setShips(prev => prev.map(s => s.id === voyageId ? { ...s, available: Math.max(0, s.available - 1) } : s));
      const updatedShip = ships.find(s => s.id === voyageId);
      if (updatedShip) persistShip({ ...updatedShip, available: Math.max(0, updatedShip.available - 1) }).catch(console.error);
    } else {
  ...
  const handleLandBookingSubmit = (e: React.FormEvent) => {
    ...
    if (isOnline) {
      ...
      if (updatedTrip) persistTrip({ ...updatedTrip, available: Math.max(0, updatedTrip.available - count) }).catch(console.error);
    } else {
      setOfflineQueue(prev => [...prev, newBookingObj]);
    }
